...
void
QRDevice::rts2Updated(QJsonDocument &doc)
{
    QJsonParseError jsonError;

    QJsonObject docObject = doc.object();
    QJsonObject devArray = docObject[thisdevice].toObject();
    QJsonObject dArray = devArray["d"].toObject();
    int r = 0;
    foreach (const QString &val, dArray.keys())
    {
	QString v = dArray[val].toArray()[1].toVariant().toString();
