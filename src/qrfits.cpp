...
void
QRFits::exposeImage(const char *device)
{
...
void
QRFits::scaleData(float min, float max, ScaleType type)
{
    if (data == NULL)
        return;
...
void
QRFits::drawImage(QPainter *painter, float x, float y)
{
    if (image == NULL)
    {
